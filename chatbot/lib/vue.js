  ...
  addHandler(el, CHECKBOX_RADIO_TOKEN,
    ...
    'if(Array.isArray($$a)){' +
      "var $$v=" + (number ? '_n(' + valueBinding + ')' : valueBinding) + "," +
          '$$i=_i($$a,$$v);' +
      "if($$c){$$i<0&&(" + value "="$$a.concat($$v))}"" "else{$$i>-1&&(" + value + "=$$a.slice(0,$$i).concat($$a.slice($$i+1)))}" +
    "}else{" + value + "=$$c}",
    null, true
  );
}
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd >= 0) {
        ...
        while (
          ...
          next = rest$1.indexOf('<', 1); if (next < 0) { break } textend +="next;" rest$1="html.slice(textEnd);" text="html.substring(0," textend); advance(textend); (textend html ; (options.chars && text) options.chars(text); else var stackedtag="lastTag.toLowerCase();" restackedtag="reCache[stackedTag]" || (recache[stackedtag]="new" regexp('([\\s\\s]*?)(< ' '[^>]*>)', 'i'));
      var endTagLength = 0;
...
</body></html></div></div></div></div></"></"></div></div></slot></"></template></"></"></"></',></\\></'></div></"></transition-group></transition></transition-group></transition></keep-alive></select></transition></transition></transition></transition></transition></0&&("></"></"></tbody></p></=></=></'></=></slot></slot></template></vnode></anonymous></anonymous></"></root>